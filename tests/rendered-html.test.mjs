...
test("exports the complete project page as static HTML", async () => {
  ...
  assert.match(html, /\/videos\/teaser-tract-20260916\.mp4/i);
  assert.doesNotMatch(html, /\/videos\/teaser\.(?:mp4|jpg)/i);
...
test("links both Code controls to the project repository", async () => {
  const html = await (await render()).text();
  const links = [...html.matchAll(/<a\b[^>]*href="https:\/\/github\.com\/brains-bots-n-behavior\/track-articulate-act"[^>]*>Code<\/a>/g)];
  assert.equal(links.length, 2);
  for (const [link] of links) {
    assert.match(link, /target="_blank"/);
    assert.match(link, /rel="noreferrer"/);
  }
  assert.doesNotMatch(html, /Code link not yet available|<button[^>]*>Code<\/button>/);
});

test("places arXiv immediately after Paper in both resource groups", async () => {
  const html = await (await render()).text();
  const groups = [
    html.match(/<nav aria-label="Primary navigation">([\s\S]*?)<\/nav>/)?.[1],
    html.match(/<div class="resource-links"[^>]*>([\s\S]*?)<\/div>/)?.[1],
  ];
  for (const group of groups) {
    assert.ok(group);
    const links = [...group.matchAll(/<a\b([^>]*)>([^<]+)<\/a>/g)];
    assert.deepEqual(links.map((link) => link[2]), ["Paper", "arXiv", "Code"]);
    assert.match(links[1][1], /href="https:\/\/arxiv\.org\/abs\/2609\.19119"/);
    assert.match(links[1][1], /target="_blank"/);
    assert.match(links[1][1], /rel="noreferrer"/);
  }
});
...
test("exports the replacement teaser with a fresh video and poster URL", async () => {
  const html = await (await render()).text();
  const teaser = html.match(/<figure class="overview-film">[\s\S]*?<\/figure>/)?.[0] ?? "";
  assert.match(teaser, /src="\/videos\/teaser-tract-20260916\.mp4"/);
  assert.match(teaser, /poster="\/videos\/teaser-tract-20260916\.jpg"/);
  for (const extension of ["mp4", "jpg"]) {
    const name = `videos/teaser-tract-20260916.${extension}`;
    assert.deepEqual(
      await readFile(new URL(`../${name}`, import.meta.url)),
      await readFile(new URL(`../public/${name}`, import.meta.url)),
      "The exported teaser must match the replacement source asset",
    );
  }
});

test("shows the RGB-D input on the left without stretching portrait video", async () => {
  const html = await (await render()).text();
  const section = html.match(/<section[^>]*id="rgbd-comparison"[\s\S]*?<\/section>/)?.[0] ?? "";
  const sources = [...section.matchAll(/<source\b[^>]*src="([^"]+)"/g)].map((match) => match[1]);
  assert.deepEqual(sources, [
    "/videos/rgbd-input.mp4",
    "/videos/retargeting/allegro/suitcase-rgb.mp4",
    "/videos/retargeting/allegro/suitcase.mp4",
  ]);
  assert.match(section, /<figcaption>RGB-D video<\/figcaption>/);
  assert.match(section, /poster="\/videos\/rgbd-input\.jpg"/);
  const css = await readFile(new URL("../app/globals.css", import.meta.url), "utf8");
  assert.match(css, /\.depth-comparison \.video-grid\s*\{[^}]*repeat\(3,/);
  assert.match(css, /\.video-frame video\s*\{[^}]*object-fit:\s*contain/);
  assert.match(css, /\.video-frame\s*\{[^}]*background:\s*#000/);
  assert.deepEqual(
    await readFile(new URL("../videos/rgbd-input.mp4", import.meta.url)),
    await readFile(new URL("../public/videos/rgbd-input.mp4", import.meta.url)),
  );
});
